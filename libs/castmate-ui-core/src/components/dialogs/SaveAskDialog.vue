<template>
	<div>
		<form @submit.prevent="saveAndClose">
			<p class="mt-0">{{ name }} {{$tSync("system.unsaved_changes")}}</p>
			<div class="flex justify-content-end mt-1 gap-1">
				<p-button size="small" text :label="$tSync('system.close')" @click="close" tabindex="1"></p-button>
				<p-button size="small" text :label="$tSync('system.cancel')" @click="cancel" tabindex="2"></p-button>
				<p-button
					size="small"
					type="submit"
					:label="$tSync('system.save_and_close')"
					tabindex="0"
					ref="saveButton"
					:autofocus="true"
				></p-button>
